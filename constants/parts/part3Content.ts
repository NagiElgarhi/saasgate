      ...
      sections: [
          ...
          content: [
            {
              ...
              text: 'إذا كنت تبني نظام SaaS، فأنت لا تُطوّر تطبيقًا عاديًا. أنت تُطوّر <b>نظامًا مركزيًا يستضيف آلاف العملاء (المستأجرين)</b>، ويُقدّم لهم نفس الخدمة، مع <b>عزل تام لبياناتهم</b>، وضمان <b>الأداء، الأمان، والتوسع</b>. السؤال الأهم ليس: "هل يمكنني بناء هذا النظام؟" بل: "هل يمكنني بناؤه بطريقة تُمكنه من النمو إلى مليون مستخدم دون الانهيار؟"',
            },
            {
                type: ContentType.PARAGRAPH,
                text: 'في هذا الفصل، سنغوص في <b>العمق التقني لتصميم نظام SaaS</b>، مع التركيز على <b>Multi-Tenant Architecture (بنية متعددة المستأجرين)</b> — وهي القلب النابض لأي نظام SaaS ناجح. هذا ليس مجرد خيار تقني، بل هو قرار استراتيجي سيحدد تكلفة تشغيلك، ومستوى أمانك، وقدرتك على النمو في المستقبل. سنتناول:'
            },
            {
              ...
              items: [
                ...
                '<b>كيف تُصمم قاعدة البيانات، الواجهة الخلفية، والواجهة الأمامية</b> لدعم المستأجرين.',
                '<b>أكواد عملية ومفصلة</b> بلغات متعددة (Python، JavaScript، SQL، Docker).',
                '<b>تحليل للمخاطر الشائعة</b> مثل "مشكلة الجار المزعج" و "جحيم ترحيل البيانات".',
        ...
        },
        {
            ...
            content: [
                ...
                { type: ContentType.PARAGRAPH, text: '<b>Multi-Tenant Architecture</b> (البنية متعددة المستأجرين) هي نموذج تصميم برمجي يُمكّن <b>تطبيقًا واحدًا من خدمة عدة عملاء (مستأجرين)</b>، مع <b>عزل بياناتهم</b> عن بعضهم البعض. المستأجر (Tenant) هو عميل أو منظمة تستخدم النظام (مثل شركة، فريق، أو مؤسسة تعليمية).'},
                { type: ContentType.NOTE, title: '🔹 تشبيه بسيط: العمارة السكنية مقابل الفيلا الخاصة', text: '- <b>Single-Tenant (المستأجر الواحد)</b> يشبه امتلاك فيلا خاصة بك. تحصل على أقصى درجات الخصوصية والأمان، وتتحكم في كل شيء، لكن التكلفة الأولية وتكاليف الصيانة باهظة جداً. كل عميل يحصل على نسخته الخاصة من التطبيق وقاعدة البيانات والخادم.\n- <b>Multi-Tenant (متعدد المستأجرين)</b> يشبه العيش في عمارة سكنية. جميع السكان يتشاركون المبنى نفسه (البنية التحتية والتطبيق)، والمصعد (الموارد)، لكن لكل ساكن شقته الخاصة وباب مقفل (عزل البيانات). التكلفة أقل بكثير لأنها موزعة على الجميع، والصيانة مركزية ومسؤولية المالك (مزود الخدمة).'},
    ...
    <div class="bg-stone-50 border-2 border-amber-400 rounded-xl p-6 shadow-lg flex flex-col">
        ...
        },
        {
            ...
            content: [
                ...
                { type: ContentType.HEADING3, text: '📌 النموذج 1: Shared Database, Shared Schema (نموذج الشقة)' },
...
SELECT * FROM tasks; -- يعرض مهام جميع المستأجرين!`,
                ...
                { type: ContentType.HEADING3, text: '📌 النموذج 2: Shared Database, Separate Schema (نموذج التاون هاوس)' },
...
SELECT * FROM tasks; -- سيقرأ من tenant_acme.tasks`,
                ...
                { type: ContentType.HEADING3, text: '📌 النموذج 3: Separate Database (نموذج الفيلا الخاصة)' },
    ...
    return create_engine(db_url)`,
        ...
        },
        {
          ...
          content: [
            ...
            { type: ContentType.DEFINITION_LIST, definitionItems: [
                { term: '1. عبر Subdomain (النطاق الفرعي)', definition: '<b>مثال: `acme.yoursaas.com`</b>. يستخرج التطبيق `acme` من عنوان URL ويستخدمه لتحديد المستأجر. هذه هي الطريقة الأكثر احترافية وشيوعًا لتطبيقات B2B.' },
                { term: '2. عبر Custom Domain (النطاق المخصص)', definition: '<b>مثال: `tasks.acme.com`</b>. يسمح للعملاء باستخدام نطاقاتهم الخاصة، مما يعزز علامتهم التجارية. يتطلب هذا إعدادات DNS معقدة.' },
                ...
                { term: '4. عبر JWT Token', definition: 'عند تسجيل الدخول، يتم تضمين `tenant_id` داخل حمولة (payload) توكن المصادقة (JWT). في كل طلب لاحق، يقوم الخادم بفك تشفير التوكن واستخراج معرف المستأجر. هذه طريقة آمنة وشائعة جدًا.' },
            ]},
            {
...
CREATE TABLE invitations (
    ...
    status VARCHAR(20) DEFAULT 'pending' -- 'pending', 'accepted'
        ...
        {
          id: 'part3_ch1_sec6',
          title: 'الأمان المتقدم: تطبيق Row-Level Security (RLS)',
          icon: '🛡️',
          content: [
            { type: ContentType.PARAGRAPH, text: 'الاعتماد على المطورين فقط لتذكر إضافة `WHERE tenant_id = ?` هو وصفة لكارثة. <b>Row-Level Security (RLS)</b> في قواعد البيانات مثل PostgreSQL هي شبكة أمان قوية تفرض عزل البيانات على مستوى قاعدة البيانات نفسها.' },
...
`,
        ...
        },
        {
          ...
          content: [
            ...
            { type: ContentType.HEADING3, text: 'التوسع الأفقي مقابل التوسع العمودي' },
            { type: ContentType.DEFINITION_LIST, definitionItems: [
        ...
        },
        {
          ...
          content: [
            { type: ContentType.DEFINITION_LIST, definitionItems: [
                { term: '1. تصميم قاعدة بيانات مع أدوار', definition: 'قم بتوسيع تصميم قاعدة بيانات تطبيق إدارة المشاريع في الفصل السابق ليشمل جدولًا لإدارة الأدوار (Roles) وجدول ربط `project_memberships` يحدد دور كل مستخدم في كل مشروع (مثل `editor`, `viewer`).' },
                { term: '2. كتابة Middleware لتحديد المستأجر', definition: 'باستخدام Node.js/Express، اكتب Middleware يستخرج النطاق الفرعي من `req.hostname`، ويبحث عن المستأجر في قاعدة بيانات وهمية، ويضيف كائن `tenant` إلى كائن `req`.' },
                { term: '3. محاكاة سياسة RLS', definition: 'إذا لم يكن لديك PostgreSQL، قم بمحاكاة سلوك RLS عن طريق إنشاء مُزيِّن (decorator) في Python/Flask يلتف حول دوال الوصول إلى قاعدة البيانات ويضيف تلقائيًا شرط `tenant_id` إلى الاستعلامات.' },
                { term: '4. تحليل بنية نظام موجود', definition: 'اختر نظام SaaS تستخدمه (مثل Figma أو Airtable). حاول تخمين نموذج عزل البيانات الذي يستخدمونه بناءً على جمهورهم المستهدف ونموذج التسعير الخاص بهم. هل هو مشترك بالكامل، أم أنهم يقدمون خطط Enterprise مع عزل أكبر؟' },
            ]}
          ]
        }
      ]
